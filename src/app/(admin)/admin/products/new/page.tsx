...
export default function AddNewProductPage() {
  ...
  const handleExtractProduct = async () => {
    if (!extractUrl.trim()) {
      showToast(lang === "ar" ? "يرجى لصق رابط المنتج أولاً" : "Please paste a product URL", "error");
      return;
    }
    ...
    try {
      const data = await productsService.extractProductInfo(extractUrl.trim());
      setProductForm((prev) => ({
        ...prev,
        name: data.title || prev.name,
        price: data.price ? String(data.price) : prev.price,
        link: extractUrl.trim(),
        colors: data.color ? [data.color] : prev.colors,
        sizes: data.size ? [data.size] : prev.sizes,
      }));
      ...
      setExtractStatus({
        text: lang === "ar" ? "تم استخراج بيانات المنتج بنجاح ✓" : "Product details extracted successfully ✓",
        isError: false,
      });
      showToast(lang === "ar" ? "تم استخراج بيانات المنتج ✓" : "Extracted successfully ✓", "success");
    } catch (err: any) {
      setExtractStatus({
        text: lang === "ar" ? "تعذر الاستخراج التلقائي. يرجى إدخال البيانات يدوياً" : "Failed to auto-extract. Please enter manually",
        isError: true,
      });
      showToast(lang === "ar" ? "فشل الاستخراج التلقائي" : "Auto-extract failed", "error");
    } finally {
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    if (!productImageFile && !extractedImageUrl) {
      showToast(lang === "ar" ? "يرجى اختيار صورة للمنتج" : "Please select a product image", "error");
      return;
    }
    ...
    try {
      ...
      if (productImageFile) {
        const uploadResult = await productsService.uploadImage(productImageFile);
        finalImageUrl = uploadResult.imageUrl;
        finalImagePath = uploadResult.imagePath;
      }

      const parsedPrice = parseFloat(productForm.price) || 0;
      const parsedOriginal = productForm.originalPrice ? parseFloat(productForm.originalPrice) : undefined;
      ...
      showToast(
        lang === "ar" ? "تم نشر المنتج بالمتجر بنجاح ✓" : "Product published to store successfully ✓",
        "success"
      );
      router.push("/admin/products");
    } catch (err) {
      console.error("Failed to add product:", err);
      showToast(lang === "ar" ? "حدث خطأ أثناء حفظ المنتج" : "Failed to save product", "error");
    } finally {
        ...
        <div className="flex flex-col gap-4 px-4 pt-1 pb-24 text-left" dir="ltr">
          ...
          <Card className="p-4 flex flex-col gap-3 bg-white border border-brand-neutral-200/90 shadow-xs">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-xl bg-primary-50 text-primary-600 flex items-center justify-center shrink-0 shadow-2xs">
                <Sparkles className="w-4 h-4" />
              </div>
              <div className="flex flex-col">
                <Heading variant="card-title" className="text-xs sm:text-sm font-bold text-brand-neutral-900">
                  {lang === "ar" ? "سحب بيانات المنتج تلقائياً (الذكاء الاصطناعي)" : "Smart Auto-Extract"}
                </Heading>
